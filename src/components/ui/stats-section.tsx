        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-5xl mx-auto">
          {perks.map((perk, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="text-center group p-6 rounded-xl border border-yellow-500/20 bg-black/40 hover:border-yellow-500/40 transition-all duration-300"
            >
              ...
              <div className="text-lg font-bold text-white mb-2">{perk.label}</div>
              <div className="text-gray-400 text-sm leading-relaxed">{perk.description}</div>
            </motion.div>
          ))}
        </div>
      </div>
