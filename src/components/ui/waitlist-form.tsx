...
function WaitlistForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!formData.email || !formData.name) { setError("Please fill in your name and email"); return }
    setIsLoading(true); setError("")
    try {
      const response = await fetch("/api/waitlist", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: formData.name, email: formData.email, phone: formData.phone ? `${formData.countryCode} ${formData.phone}` : "" }),
      })
      if (response.ok) { setIsSubmitted(true) } else { throw new Error("Failed to join waitlist") }
  ...
  if (isSubmitted) {
    return (
      <div id="waitlist" className="h-[40rem] w-full rounded-2xl relative flex flex-col items-center justify-center antialiased border border-white/10 overflow-hidden" style={{ background: "rgba(33,33,33,0.6)" }}>
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="max-w-2xl mx-auto p-4 text-center">
  ...
  return (
    <div
      id="waitlist"
      className="min-h-[42rem] w-full rounded-2xl relative flex flex-col items-center justify-center antialiased py-12 border border-white/10 overflow-hidden"
    ...
    >
      <div className="max-w-xl mx-auto p-4 w-full">
        ...
        >
          <div className="inline-flex items-center gap-2 border border-blue-500/20 rounded-full px-4 py-2 mb-6" style={{ background: "rgba(23,74,255,0.09)" }}>
            <Sparkles className="w-4 h-4 text-[#174AFF]" />
          ...
          <p className="text-gray-400 text-base font-manrope">Be first in line when Digital Realtor launches.</p>
        </motion.div>
        ...
        >
          <div className="relative">
            ...
            <Input type="text" placeholder="Your full name *" value={formData.name}
              ...
              className="pl-10 h-12 bg-white/5 border-white/10 text-white placeholder:text-gray-500 focus:border-[#174AFF] focus:ring-[#174AFF]/20 font-manrope" required />
          </div>
          <div className="relative">
          ...
          <div className="flex gap-2">
            <div className="relative">
              ...
              <select value={formData.countryCode} onChange={(e) => setFormData({ ...formData, countryCode: e.target.value })}
                ...
                {COUNTRY_CODES.map((c, i) => <option key={i} value={c.code} className="bg-[#0B0B0B]">{c.flag} {c.code}</option>)}
              </select>
            </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.5 }}
          ...
          <div className="flex items-center justify-center gap-4 text-xs text-gray-500 font-manrope">
            <span>No spam, ever</span><span>·</span><span>Unsubscribe anytime</span><span>·</span><span>Early access priority</span>
          </div>
        </motion.div>
      </div>
      <BackgroundBeams />
